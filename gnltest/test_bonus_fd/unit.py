...
#		############################# BUFF_SIZE 1 #############################
		...
		if nolib == 1:
			if not self.execute("clang -Wall -Werror -Wextra -o get_next_line.o -c get_next_line.c && clang -Wall -Werror -Wextra -o main.o -c main.c && clang -Wall -Werror -Wextra -o test_multiple_fd.o -c test_multiple_fd.c && clang -o test_gnl test_multiple_fd.o main.o get_next_line.o", "compil_test_output", required_files=corrfiles + srcfiles, trace_output=True, trace_cmd=False, copyback_files=["test_gnl"], maxtime=30):
...
#		############################ BUFF_SIZE 2056 ############################
		...
		self.trace("%d/%d correct tests" % (count, 4))
		self.comment("%d/%d correct test" % (count, 4))
		self.set_grade(count * 15 / 4)
		if count != 4:
			self.ko()
